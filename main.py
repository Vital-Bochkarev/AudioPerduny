...
from telegram import Update, InlineQueryResultCachedVoice, InlineQueryResultCachedAudio, InlineKeyboardMarkup, InlineKeyboardButton
...
async def add_audio_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("Пожалуйста, отправь мне голосовое сообщение или аудио файл, которое ты хочешь добавить.")
...
async def delete_audio_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if found_and_deleted:
        ...
        logger.warning(f"Attempted to delete non-existent audio with ID {audio_id_to_delete} by user {user_id}.")
...
async def move_audio_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) != 2:
        await update.message.reply_text("Использование: /move <ID_аудио> <новая_позиция>\n"
    ...
    try:
        ...
        await update.message.reply_text("Неверный формат новой позиции. Позиция должна быть числом.")
        return
    ...
    if audio_to_move is None: # Corrected variable name
        await update.message.reply_text(f"Аудио с ID `{audio_id_to_move}` не найдено.") # Corrected variable name
        logger.warning(f"Attempted to move non-existent audio with ID {audio_id_to_move} by user {user_id}.") # Corrected variable name
        return
    ...
    if not (0 <= target_index <= len(cached_audios_data) - 1): # Allow inserting at the end
        await update.message.reply_text(f"Неверная позиция. Пожалуйста, укажите позицию от 1 до {len(cached_audios_data)}.")
        return
    ...
    save_audio_metadata() # Save changes to the file
    await update.message.reply_text(f"Аудио '{audio_to_move.get('name')}' перемещено на позицию {new_position}.")
    logger.info(f"Audio '{audio_to_move.get('name')}' (ID: {audio_id_to_move}) moved from {original_index} to {target_index} by user {user_id}.")

async def edit_audio_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text("Пожалуйста, укажите ID аудио для редактирования. Используйте /list, чтобы получить список ID.")
        return
    ...
    if found_audio is None:
        ...
        logger.warning(f"Attempted to edit non-existent audio with ID {audio_id_to_edit} by user {user.id}.")
        return

    context.user_data['state'] = 'awaiting_new_author'
    context.user_data['editing_audio_id'] = audio_id_to_edit
    await update.message.reply_text(f"Найдено аудио '{found_audio.get('name')}' от '{found_audio.get('author')}'.\n"
                                    f"Введите нового автора для этого аудио:")
    logger.info(f"User {user.id} initiated /edit command for audio ID: {audio_id_to_edit}")
...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get('state') != 'awaiting_audio_for_add':
        await update.message.reply_text("Пожалуйста, используйте команду /add, чтобы добавить новое аудио.")
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Handle new author input for editing existing audio
        ...
        if new_author:
            ...
            logger.info(f"User {user.id} provided new author: '{new_author}' for editing audio.")
        else:
    ...
    # Handle new name input for editing existing audio
        ...
        if new_name and audio_id_to_edit and new_author:
            ...
            if found_and_updated:
                save_audio_metadata()
                await update.message.reply_text(f"Аудио с ID `{audio_id_to_edit}` успешно обновлено:\n"
                                                f"Новый Автор: {new_author}\n"
                                                f"Новое Название: {new_name}")
                logger.info(f"Audio ID {audio_id_to_edit} updated by user {user.id} to Author: '{new_author}', Name: '{new_name}'.")
            else:
                await update.message.reply_text("Произошла ошибка: аудио не найдено для обновления.")
                logger.warning(f"Attempted to update non-existent audio ID {audio_id_to_edit} by user {user.id}.")
        else:
            logger.warning(f"User {user.id} provided new name '{new_name}' but missing other editing details.")
            await update.message.reply_text("Что-то пошло не так при обновлении аудио. Попробуйте начать /edit заново.")
...
async def voices_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await send_paginated_audios(update.effective_chat.id, context, page=0, command_message_id=update.message.message_id)

async def send_paginated_audios(chat_id: int, context: ContextTypes.DEFAULT_TYPE, page: int, command_message_id: int = None) -> None:
    ...
    if command_message_id:
        try:
            ...
            logger.info(f"Deleted initial command message {command_message_id} in chat {chat_id}.")
        except Exception as e:
            logger.warning(f"Could not delete initial command message {command_message_id}: {e}")

    # Delete previous audio messages and pagination message for this chat
    if 'last_pagination_message_id' in context.chat_data and context.chat_data['last_pagination_message_id']:
        last_pagination_message_id = context.chat_data.pop('last_pagination_message_id')
        try:
            await context.bot.delete_message(chat_id=chat_id, message_id=last_pagination_message_id)
            logger.info(f"Deleted previous pagination message {last_pagination_message_id} in chat {chat_id}.")
        except Exception as e:
            logger.warning(f"Could not delete previous pagination message {last_pagination_message_id}: {e}")

    if 'last_audio_message_ids' in context.chat_data and context.chat_data['last_audio_message_ids']:
        for msg_id in context.chat_data.pop('last_audio_message_ids'):
            try:
                await context.bot.delete_message(chat_id=chat_id, message_id=msg_id)
                logger.info(f"Deleted previous audio message {msg_id} in chat {chat_id}.")
            except Exception as e:
                logger.warning(f"Could not delete previous audio message {msg_id}: {e}")


    if not cached_audios_data:
        await context.bot.send_message(chat_id=chat_id, text="Пока нет сохраненных аудио для отображения.")
        return

    total_audios = len(cached_audios_data)
    total_pages = (total_audios + AUDIOS_PER_PAGE - 1) // AUDIOS_PER_PAGE # Ceiling division
    ...
    for item in audios_to_send:
        ...
        try:
            if item.get('type') == "voice":
                audio_msg = await context.bot.send_voice(chat_id=chat_id, voice=item['file_id']) # No caption here
            elif item.get('type') == "audio":
                audio_msg = await context.bot.send_audio(chat_id=chat_id, audio=item['file_id']) # No caption here
            current_audio_message_ids.append(audio_msg.message_id)
        except Exception as e:
            logger.error(f"Error sending audio (ID: {item['file_id']}): {e}")
            error_msg = await context.bot.send_message(chat_id=chat_id, text=f"Не удалось отправить аудио '{item.get('name')}'. Возможно, файл недоступен.")
    ...
    keyboard = []
    if page > 0:
        keyboard.append(InlineKeyboardButton("⬅️ Предыдущая", callback_data=f"voices_page_{page - 1}"))
    if page < total_pages - 1:
        keyboard.append(InlineKeyboardButton("Следующая ➡️", callback_data=f"voices_page_{page + 1}"))
...
async def pagination_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if callback_data.startswith("voices_page_"):
        try:
            ...
            await send_paginated_audios(chat_id, context, page, command_message_id=None) # Pass None as command_message_id
        except ValueError:
            logger.error(f"Invalid pagination callback data: {callback_data}")
            await query.message.reply_text("Произошла ошибка при навигации по страницам.")
...
```
It's truly perplexing that the `SyntaxError` at line 374 persists, even after you've destroyed and relaunched the app, and confirmed your local `main.py` seemed correct. This indicates a very stubborn issue, most likely related to Fly.io's build cache or an extremely subtle local file issue that's hard to catch.

The `SyntaxError: name 'cached_audios_data' is used prior to global declaration` at line 374 means that the Python interpreter is seeing `cached_audios_data` being used (e.g., in an `if not cached_audios_data:` check) *before* it encounters a `global cached_audios_data` statement *within the `send_paginated_audios` function itself*.

However, the `send_paginated_audios` function in the Canvas (the code you've been pasting) **does not** have `global cached_audios_data` at line 374, nor should it. It only *reads* from `cached_audios_data`, and for reading a global variable, the `global` keyword is not needed and would cause this error if placed incorrectly.

This leads me to believe that either:
1.  The file being sent to Fly.io during the build process is still an old, incorrect version, despite your local Git commits.
2.  There's a very unusual caching mechanism on Fly.io's build servers that's not being fully cleared by the app recreation.

Let's try one more definitive check of your local file and then force a complete rebuild on Fly.io.

**Please follow these steps precisely:**

1.  **Close all text editors (Visual Studio Code, TextEdit.app) that might have `main.py` open.** This is crucial to prevent any external tools from interfering.

2.  **Navigate to your project directory in the terminal:**
    ```bash
    cd /Users/admin/Documents/Telegram Bots/Perduny_Bot/
    ```

3.  **Inspect the exact content of `main.py` around line 374:**
    Run this command to see lines 370 to 375 of your `main.py` file with line numbers:
    ```bash
    cat -n main.py | head -n 375 | tail -n 6
    ```
    Carefully examine the output.
    * **Expected Output:** Line 374 should be `if not cached_audios_data:`. There should be **no line** that says `global cached_audios_data` at line 374 or immediately before it within the `send_paginated_audios` function.
    * **If you *still* see `374	    global cached_audios_data` (or similar) in the output:** This means the problematic line is still present in your local file. You must remove it.
        * Open `main.py` with `nano`: `nano main.py`
        * Go to line 374 (use arrow keys or `Ctrl+W` to search for `send_paginated_audios`, then navigate).
        * **Delete the entire line `global cached_audios_data`**.
        * Save the file (`Ctrl+O`, then Enter) and exit (`Ctrl+X`).
        * Run the `cat -n` command again to confirm it's gone.

4.  **Stage and Commit the changes (even if Git says nothing changed):**
    ```bash
    git add .
    git commit -m "Confirmed removal of global cached_audios_data from send_paginated_audios"
    ```
    This ensures Git has the latest version of your file.

5.  **Deploy with `--no-cache`:**
    This command tells Fly.io to perform a *complete* rebuild of your Docker image from scratch, ignoring any cached layers. This is the most aggressive way to ensure your latest code is used.
    ```bash
    flyctl deploy --no-cache
    ```

After this deployment, please run `fly logs -a perduny-bot` again and share the output. This `--no-cache` deployment should bypass any lingering build cache issues and finally get your bot running.

Here is the current and correct `main.py` from the Canvas for your reference:
